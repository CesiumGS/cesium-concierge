...
var defaultLabelBump = fs.readFileSync(path.join(__dirname, 'templates', 'labelBump.hbs')).toString();
...
function RepositorySettings(options) {
    ...
    this.labelBumpTemplate = defaultValue(options.labelBumpTemplate, defaultLabelBump);
    ...
    /**
     * Gets the dictionary of labels to bump.
     */
    this.labelsToBump = options.labelsToBump;
...
Object.defineProperties(RepositorySettings.prototype, {
    ...
    /**
     * Gets the handlebars template to use when bumping labelled issues.
     * @type {String}
     */
    labelBumpTemplate: {
        get: function () {
            return handlebars.compile(this._labelBumpTemplate + this.signatureTemplate);
        },
        set: function (value) {
            this._labelBumpTemplate = value;
        }
    },
