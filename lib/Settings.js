...
function dumpEnvironment(object, parentName) {
    var envString = '';
    Object.keys(object).forEach(function(name) {
        var fullName = parentName === '' ? name : parentName + '__' + name;
        if (typeof object[name] === 'object') {
            var string = dumpEnvironment(object[name], fullName);
            envString += string;
        } else {
            envString += fullName + ' = ' + object[name] + '\n';
        }
    });
    return envString;
}
...
Settings.loadRepositoriesSettings = function (configPath) {
    ...
    // Save the configuration to disk so that cron scripts can read from it
    var environmentString = dumpEnvironment(nconf.get(), '');
    fs.writeFile('saved_env', environmentString);
