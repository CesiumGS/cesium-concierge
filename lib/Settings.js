...
Settings.loadRepositoriesSettings = function (configPath) {
    ...
    for (var i = 0; i < repositoryNames.length; i++) {
        ...
        var thirdPartyFolders = configJson[name].thirdPartyFolders;
        ...
        if (parseBoolean(configJson[name].checkCLA) && fsExtra.pathExistsSync('./CLA.json')) {
            configJson[name].claJson = fsExtra.readJsonSync('./CLA.json');
        } else {
            dateLog('Could not find `./CLA.json` or `checkCLA` is `false`.');
        }
    }
