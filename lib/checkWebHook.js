...
function checkWebHook(req, res, next) { // eslint-disable-line no-unused-vars
    ...
    if (!defined(id)) {
        throw new Error('No id found in the request');
    }

    var event = req.headers['x-github-event'];
    if (!defined(event)) {
        throw new Error('No event found in the request');
    }

    var sign = req.headers['x-hub-signature'] || '';
    if (Settings.secret && !sign) {
        throw new Error('No signature found in the request');
    }

    if (!defined(req.body)) {
        throw new Error('Make sure body-parser is used');
    }
