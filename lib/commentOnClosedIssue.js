...
commentOnClosedIssue._implementation = function (issueUrl, commentsUrl, headers) {
    ...
    .then(function (issueResponse) {
        if (!statusCodeIsOk(issueResponse)) {
            return Promise.reject('Did not receive response code 200 for url:', issueUrl);
        }
    ...
    .then(function (commentsJsonResponse) {
        if (!statusCodeIsOk(commentsJsonResponse)) {
            return Promise.reject('Did not receive response code 200 for url:', commentsUrl);
        }
        comments.concat(commentsJsonResponse.body.map(function (commentJson) {
...
function statusCodeIsOk(response) {
    console.log(new Date(Date.now()).toISOString() + ' Received from server:', response);
    if (response.statusCode === 200) {
        console.log(new Date(Date.now()).toISOString() + ' Status code OK 200');
        return true;
    }
    console.log(new Date(Date.now()).toISOString() + ' Status code ERROR: ' + response.statusCode + ', ' + response.statusMessage);
    return false;
}
