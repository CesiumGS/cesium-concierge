...
commentOnOpenedPullRequest._implementation = function (pullRequestFilesUrl, pullRequestCommentsUrl, repositorySettings, userName, repositoryUrl, baseBranch, headBranch) {
    ...
    return repositorySettings.fetchSettings()
        ...
        .then(function () {
            return commentOnOpenedPullRequest._isUserNameInContributors(userName, repositorySettings, headBranch);
        })
        .then(function (result) {
            askAboutContributors = !result;
        })
        .then(function () {
            return commentOnOpenedPullRequest._isUserNameInContributors(userName, repositorySettings, baseBranch);
        })
        .then(function (result) {
            firstTimeContributor = !result;
        })
        .then(function () {
        ...
        .then(function (filesJsonResponse) {
            ...
            var message = repositorySettings.pullRequestOpenedTemplate({
                ...
                firstTimeContributor: firstTimeContributor
            });
...
commentOnOpenedPullRequest._isUserNameInContributors = function (userName, repositorySettings, branch) {
    if (!defined(repositorySettings.contributorsUrl)) {
        return Promise.resolve(false);
    }

    return requestPromise.get({
        url: repositorySettings.contributorsUrl + '?ref=' + branch,
        headers: repositorySettings.headers,
        json: true
    })
