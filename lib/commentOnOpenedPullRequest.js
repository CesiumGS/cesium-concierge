...
function commentOnOpenedPullRequest(body, repositorySettings) {
    ...
    return commentOnOpenedPullRequest._implementation(filesUrl, commentsUrl, repositorySettings, userName, repositoryUrl, baseBranch);
}
...
commentOnOpenedPullRequest._askAboutChanges = function (files, baseBranch) {
    var changesMDWasUpdated = false;
    for (var i = 0; i < files.length; i++) {
        if (/^CHANGES\.md$/.test(files[i])) {
            changesMDWasUpdated = true;
        }
    }
    var isMergingIntoMaster = (baseBranch === 'master');
    return !changesMDWasUpdated && isMergingIntoMaster;
};
