 ...
 */
function commentOnOpenedPullRequest(jsonResponse, headers, changesSignature, thirdPartySignature, thirdPartyFolders) {
    ...
    if (!defined(pullRequest)) {
        return Promise.reject('jsonResponse has no key `pull_request`: ' + JSON.stringify(jsonResponse));
    }
    return commentOnOpenedPullRequest._implementation(pullRequest.url + '/files', pullRequest.comments_url, headers, changesSignature, thirdPartySignature, thirdPartyFolders);
}
...
commentOnOpenedPullRequest._implementation = function (pullRequestFilesUrl, pullRequestCommentsUrl, headers,
                                                       changesSignature, thirdPartySignature, thirdPartyFolders) {
    ...
    .then(function (filesJsonResponse) {
        ...
        return Promise.resolve('Did not change CHANGES.md or any third party files');
    });
};
