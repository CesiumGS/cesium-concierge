...
/**
 * Transform a GitHub API URL to a regular GitHub URL.
 *
 * @param {String} apiURL The GitHub API URL.
 * @returns {String} A regular GitHub URL.
 */
function getURLFromAPIUrl(apiURL) {
    var finalURL = apiURL.replace('api.','');
    finalURL = finalURL.replace('repos/','');
    finalURL = finalURL.replace('contents/','blob/master/');
    return finalURL;
}
    ...
    return repositorySettings.fetchSettings()
        ...
        .then(function (filesJsonResponse) {
            ...
            if (defined(repositorySettings.contributorsUrl)) {
                contributorsUrl = getURLFromAPIUrl(repositorySettings.contributorsUrl);
            }
